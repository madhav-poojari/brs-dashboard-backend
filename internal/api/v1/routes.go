...
func (a *API) routes() {
	...
	r.Route("/payouts", func(r chi.Router) {
		...
		// Admin-only routes
		adminGroup := r.With(auth.AuthMiddleware(a.store)).With(auth.RoleMiddleware("admin"))
		adminGroup.Get("/pending", payoutH.ListPending)
		adminGroup.Post("/approve/{id}", payoutH.ApproveTransaction)
		adminGroup.Post("/reject/{id}", payoutH.RejectTransaction)
		adminGroup.Get("/balances", payoutH.ListBalances)
		adminGroup.Post("/adjust", payoutH.AdminAdjust)
		adminGroup.Get("/timeline/{userId}", payoutH.GetTimeline)
		adminGroup.Post("/trigger-deduction", payoutH.TriggerDeduction)
	})
