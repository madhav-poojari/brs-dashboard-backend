...
func NewGormStore(cfg *config.Config) (*Store, error) {
	...
	if err := db.Set("gorm:DisableForeignKeyConstraintWhenMigrating", true).AutoMigrate(&models.User{}, &models.UserDetails{}, &models.RefreshToken{}, &models.Relation{}, &models.LessonPlan{}, &models.Note{}); err != nil {
		return nil, err
	}
	sqlDB, err := db.DB()
	if err != nil {
